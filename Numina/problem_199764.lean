...
theorem number_theory_199764 (l : List ℕ) (hl : l ~ List.Ico 111 1000) :
    37 ∣ Nat.ofDigits 10 (l.map (Nat.digits 10)).flatten := by
  generalize hn : Nat.ofDigits 10 (l.map (Nat.digits 10)).flatten = n

  have : Nat.ofDigits 10 (List.map (Nat.digits 10) l).flatten = Nat.ofDigits 1000 l := by
    suffices ∀ t : List ℕ, (∀ x ∈ t, 100 ≤ x ∧ x < 1000) →
        Nat.ofDigits 10 (List.map (Nat.digits 10) t).flatten = Nat.ofDigits 1000 t by
      refine this l ?_
      refine fun x hx ↦ ?_
      rw [hl.mem_iff, List.Ico.mem] at hx
    ...
    | nil => simp
    | cons x l IH =>
      rw [List.map_cons, List.flatten_cons, Nat.ofDigits_append, Nat.ofDigits_cons]
  ...
  · intro x hx
    ...
    exact hx.2
  · intro hl_ne
    have : 0 ∉ l := by simp [hl.mem_iff]
    refine mt (fun h ↦ ?_) this
    simpa [h] using List.getLast_mem hl_ne

  suffices l.sum = (111 + 999) * 889 / 2 from this ▸ Nat.dvd_mul_left 37 (15 * 889)
  calc _
  _ = (List.Ico 111 1000).sum := hl.sum_eq
