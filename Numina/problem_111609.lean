...
theorem algebra_111609 {a b c d : ℝ} (ha : a ≠ 0)
  ...
  -- This is zero at `x = -1` and `x = 1`, negative in `(-1, 1)`, and positive elsewhere.
  ...
  -- Obtain IsMaxOn for `-2 ≤ x ≤ 0`. This will be combined with strict concavity for uniqueness.
  ...
  -- Use this to split the interval as needed.
  ...
  · intro x hx
    ...
    cases hx with
    | inl hx =>
      -- Use concavity and first, second derivatives to show that maximum is attained at (-1).
      calc _
      _ ≤ f.eval (-1) := h_isMaxOn_neg_one hx
      _ = _ := by norm_num [hf]  -- TODO: this everywhere
    | inr hx =>
      -- Use convexity to show that max is attained at an endpoint.
      calc _
  ...
  · ext x
    ...
    · intro ⟨hx, h⟩
      ...
      refine hx.imp ?_ ?_
      · intro hx
        -- Use strict concavity combined with (-1) being maximizer to show uniqueness.
        replace h_concave := h_concave.subset (Set.Icc_subset_Iic_self) (convex_Icc (-2) 0)
        refine h_concave.eq_of_isMaxOn ?_ ?_ hx (by norm_num)
        · intro a ha
          ...
          _ = 62 := by norm_num [hf]
          _ = _ := h.symm
        · exact h_isMaxOn_neg_one
      · -- Use strict convexity to show strictly less than the max endpoint.
